...
| Escrita sem `WITH CHECK` | `alto` | policy de UPDATE/ALL só com `USING` | Existe trigger `BEFORE INSERT/UPDATE` que valida o dono, ou a coluna de dono tem `DEFAULT auth.uid()` e não é atualizável. Citar a migration do trigger. |
