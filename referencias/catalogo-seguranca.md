...
| Policy de UPDATE deixa mudar coluna de sistema | `critico` | `WITH CHECK` não impede alterar `role`, `plano`, `owner_id`, `status`, `tipo_acesso` | Trigger `BEFORE UPDATE` barra a mudança da coluna. Citar arquivo e linha. Detector mecânico: `coluna-de-privilegio-editavel` (só acusa coluna que uma função de acesso lê, e reconhece coluna nascida por `ADD COLUMN`). |
...
| RPC aceita id arbitrário sem checar vínculo | `critico` | função recebe id e não confronta com `auth.uid()` nem com o vínculo | A função é definer **e** valida o vínculo internamente (citar a linha do `where`), ou só é chamável por staff (citar o grant/policy). Detector mecânico: `definer-confia-no-parametro`. **Armadilha que ele pega e que revisão humana deixa passar:** `revoke ... from public, anon` NÃO tira o `authenticated`, que recebe EXECUTE por *default privileges* no Supabase; o fechamento certo cita os três. Num retrato do banco (pg_dump, baseline consolidado) a lista de GRANT é completa: quem não aparece não tem acesso. Função sobrecarregada vale por versão, não por nome. |
...
| Acesso por id sem conferir o dono (IDOR no servidor) | `alto` gravando, `medio` lendo | procedimento tRPC, action ou handler POST/PUT/PATCH/DELETE que recebe o id no input e lê ou grava sem citar o usuário da sessão nem chamar função que confira posse. Estar logado (`protectedProcedure`) não diz de quem é o objeto. Detector mecânico: `acesso-por-id-sem-dono` | A posse é conferida num middleware, numa policy do banco (cliente com a sessão do usuário) ou numa função chamada; ou o id é um token secreto que já é a autorização (convite, link de download). Citar arquivo e linha. |
