...
O `verificar.py` confere 36 detectores (cada um com um caso limpo que ele NÃO pode acusar, e cada condição testada por mutação), o inventário medido, a prova de cobertura e 5 propriedades do painel. Se ele imprime `APROVADO`, a ferramenta está funcionando.
...
- **Calibrado em app real, e ainda com ruído.** O motor rodou em quatro apps grandes de código aberto em Supabase (centenas de tabelas, mais de mil migrations) e uma amostra dos achados foi julgada por revisores independentes lendo o código: depois da calibragem ele mantém todos os achados verdadeiros da amostra e cerca de 45% do que acusa é real. Uma segunda rodada usou seis apps com correção de segurança publicada, rodando o motor antes e depois de cada correção e contra os commits posteriores dos autores. Os três detectores nascidos ali (função privilegiada que confia no id recebido, coluna de privilégio que o dono da linha edita, e handler de servidor que lê ou grava por id sem conferir o dono) acusaram as funções e rotas que esses autores depois corrigiram, e numa amostra julgada por céticos em apps sem correção publicada cerca de três em cada quatro acusações eram reais, e o resto era quase todo acesso público intencional. O scanner como um todo segue mais ruidoso que isso. É por isso que o método tem a fase dos céticos: o scanner é localizador, não veredito.
